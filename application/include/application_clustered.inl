...
void ApplicationClustered<T>::updateResourceCommandBuffers(FrameResource& res) {
  ...
  res.commandBuffer("compute")->bindDescriptorSets(
  ...
  res.commandBuffer("gbuffer")->bindDescriptorSets(vk::PipelineBindPoint::eGraphics, this->m_pipelines.at("scene").layout(), 0, {this->m_descriptor_sets.at("matrix"), this->m_descriptor_sets.at("textures")}, {});
  ...
  res.commandBuffer("gbuffer").bindGeometry(m_model);
  res.commandBuffer("gbuffer").drawGeometry(1);
  ...
  res.commandBuffer("lighting").imageBarrier(this->m_images.at("light_vol"), vk::ImageLayout::eGeneral, 
