...
void ApplicationVulkan::recordDrawBuffer(FrameResource& res) {
  ...
  vk::ImageBlit blit{};
  blit.srcSubresource = m_images.at("color_2").view().resourceLayer();
  ...
  res.command_buffers.at("draw")->blitImage(m_images.at("color_2").get(), m_images.at("color_2").view().layout(), m_swap_chain.image(res.image), vk::ImageLayout::eTransferDstOptimal, {blit}, vk::Filter::eNearest);
