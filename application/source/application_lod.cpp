...
ApplicationLod::ApplicationLod(std::string const& resource_path, Device& device, Surface const& surf, cmdline::parser const& cmd_parse) 
...
void ApplicationLod::recordDrawBuffer(FrameResource& res) {
  ...
  res.commandBuffer("draw")->blitImage(m_images.at("color").get(), vk::ImageLayout::eTransferSrcOptimal, m_swap_chain.image(res.image), vk::ImageLayout::eTransferDstOptimal, {blit}, vk::Filter::eNearest);
