 ...
 ,m_size_node{sizeof(serialized_triangle) * m_bvh.get_primitives_per_node()}
{
  ...
  for(std::size_t i = 0; i < m_num_uploads; ++i) {
    m_buffers_stage.emplace_back(device.createBuffer(m_size_node, vk::BufferUsageFlagBits::eTransferSrc, vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent));
  }
  ...
  m_memory_stage = Memory{device, requirements_stage, m_buffers_stage.front().memFlags()};
  for(auto& buffer : m_buffers_stage) {
    buffer.bindTo(m_memory_stage);
  }
  // create drawing memory and buffers
  for(std::size_t i = 0; i < num_nodes; ++i) {
    m_buffers.emplace_back(device.createBuffer(m_size_node, vk::BufferUsageFlagBits::eVertexBuffer | vk::BufferUsageFlagBits::eTransferDst, vk::MemoryPropertyFlagBits::eDeviceLocal));
  }
  ...
  m_memory = Memory{device, requirements_draw, m_buffers.front().memFlags()};
